...
def board():
    ...
    USER_INP = simpledialog.askstring(title="Good job!",
...
def donothing():
    ...
    student = []
    studentSize = 15
    student.append(canvas.create_rectangle(studentSize,studentSize, studentSize * 2, studentSize * 2, fill="white" ))
...
def placeParty():
    ...
    while overlapping(partyPos, fencePos1) or overlapping(partyPos, fencePos2) or overlapping(partyPos, fencePos3) or overlapping(partyPos, fencePos4) or overlapping(partyPos, fencePos5):
            partyX = random.randint(0,width-studentSize)
            partyY = random.randint(0,height-studentSize)
            canvas.move(party, partyX, partyY)
            partyPos = canvas.coords(party)
    ...
    while positions[0][0] < 0 or positions[0][2] > width or positions[0][3] > height or positions[0][1] < 0:
            partyX = random.randint(0,width-studentSize)
            partyY = random.randint(0,height-studentSize)
            canvas.move(party, partyX, partyY)
    positions.clear()
...
def placeGuards():
    ...
    guard = canvas.create_rectangle(0,0, studentSize*2, studentSize*2, fill="darkblue" )
    ...
    while overlapping(partyGuard, Spos) or overlapping(partyGuard, partyPos) or overlapping(partyGuard, fencePos1) or overlapping(partyGuard, fencePos2) or overlapping(partyGuard, fencePos3) or overlapping(partyGuard, fencePos4) or overlapping(partyGuard, fencePos5):
            guardX = random.randint(0,width-studentSize)
            guardY = random.randint(0,height-studentSize)
            canvas.move(guard, guardX, guardY)
            partyGuard = canvas.coords(guard)
    guard1 = canvas.create_rectangle(0,0, studentSize*2, studentSize*2, fill="darkblue" )
    ...
    while overlapping(partyGuard, Spos) or overlapping(partyGuard, partyPos) or overlapping(partyGuard, fencePos1) or overlapping(partyGuard, fencePos2) or overlapping(partyGuard, fencePos3) or overlapping(partyGuard, fencePos4) or overlapping(partyGuard, fencePos5):
        ...
        canvas.move(guard1, guard1X, guard1Y)
        partyGuard = canvas.coords(guard1)
    guard2 = canvas.create_rectangle(0,0, studentSize*2, studentSize*2, fill="darkblue" )
...
def placeFence():
    ...
    fencePos4 = canvas.coords(fence4)
    while overlapping(fencePos1, fencePos4) or overlapping(fencePos2, fencePos4) or overlapping(fencePos3, fencePos4):
    ...
    fencePos5 = canvas.coords(fence5)
    while overlapping(fencePos1, fencePos5) or overlapping(fencePos2, fencePos5) or overlapping(fencePos3, fencePos5)  or overlapping(fencePos4, fencePos5) :
...
def HideKey(event):
    ...
    canvas.create_text(width/2,height/2,fill="white",font="Corbel 30 italic bold", text="Busted!Exit Game!")
def Show():
    window.deiconify()
...
def moveGuard():
    ...
    while overlapping(GuardPos, partyPos) or overlapping(GuardPos, fencePos1) or overlapping(GuardPos, fencePos2) or overlapping(GuardPos, fencePos3) or overlapping(GuardPos, fencePos4) or overlapping(GuardPos, fencePos5):
            guardX = random.randint(0,width-studentSize)
            guardY = random.randint(0,height-studentSize)
            canvas.move(guard, guardX, guardY)
            GuardPos = canvas.coords(guard)
...
def moveStudent():
    ...
    if positions[0][0] < 0:
        canvas.coords(student[0],width,positions[0][1],width-studentSize,positions[0][3])
        ...
        canvas.coords(student[0],positions[0][0],height,positions[0][2],height-studentSize)
    positions.clear()
    positions.append(canvas.coords(student[0]))
    ...
    if overlapping(sHeadPos, GuardPos):
        gameOver = True
        canvas.create_text(width/2,height/2,fill="white",font="corbel 20 italic bold", text="Game over! Close the program." ,tag="text")
    GuardPos = canvas.coords(guard1)
    if overlapping(sHeadPos, GuardPos):
        gameOver = True
        canvas.create_text(width/2,height/2,fill="white",font="corbel 20 italic bold", text="Game over! Close the program.", tag="text")
...
score = 0
txt = "Score:" + str(score)
scoreText = canvas.create_text( width/2 , 10 , fill="white" , font="corbel 20 italic bold", text=txt, tag = 'score')
